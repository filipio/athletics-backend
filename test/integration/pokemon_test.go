...
func TestGetPokemonByName(t *testing.T) {
	t.Run("GET pokemon by name", testCase(func(t *testing.T) {
		...
		response, paginatedResponse, err := Get[httpio.PaginatedResponse](fmt.Sprintf("/api/v1/pokemons?name=%s", "Pikachu"))
...
func TestDeletePokemon(t *testing.T) {
	t.Run("DELETE pokemon", testCase(func(t *testing.T) {
		...
		response, parsedResponse, err := Delete[httpio.AnyMap](fmt.Sprintf("/api/v1/pokemons/%d", pokemon.ID))
