...
func (a *AnswerOfQuestion) Validate(questionType string) error {
	...
	if err := httpio.Validate(answer); err != nil {
		return errors.New("invalid json")
	}
...
func (m Answer) Validate(db *gorm.DB) error {
	...
	if err := m.Content.Validate(question.Type); err != nil {
		return httpio.AppValidationError{
			FieldPath: "content",
			AppError:  httpio.AppError{Message: err.Error()},
		}
	}
...
func onlyCurrentUserRecords(db *gorm.DB, r *http.Request) *gorm.DB {
	onlyForCurrentUser := r.Context().Value(httpio.OnlyCurrentUserContextKey).(bool)
	if onlyForCurrentUser {
		currentUser := r.Context().Value(httpio.UserContextKey).(User)
