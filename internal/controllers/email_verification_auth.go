...
func RequestVerification(deps *config.Dependencies) httpio.HandlerWithError {
	return httpio.HandlerWithError(
		func(w http.ResponseWriter, r *http.Request) error {
			db := deps.DB
			payload, err := httpio.DecodeAndValidate[RequestVerificationPayload](r, db)
			if err != nil {
				return err
			}
			...
			if existingUser.ID != 0 {
				return httpio.EmailAlreadyExistsError{}
			}
			...
			if !rateLimit.CanRequestVerification() {
				...
				return httpio.EmailVerificationRateLimitError{
			...
			if emailErr != nil {
				return httpio.EmailSendError{
					OriginalError: emailErr,
				}
			}

			httpio.Encode(w, r, http.StatusOK, httpio.AnyMap{
...
func VerifyEmail(deps *config.Dependencies) httpio.HandlerWithError {
	return httpio.HandlerWithError(
		func(w http.ResponseWriter, r *http.Request) error {
			db := deps.DB
			payload, err := httpio.DecodeAndValidate[VerifyEmailPayload](r, db)
			if err != nil {
				return err
			}
			...
			if pendingReg.ID == 0 {
				return httpio.InvalidVerificationTokenError{}
			}
			...
			err = db.Transaction(func(tx *gorm.DB) error {
				...
				if err := tx.Where("name = ?", httpio.UserRole).First(&role).Error; err != nil {
					return err
				}
			...
			httpio.Encode(w, r, http.StatusOK, tokenPair)
