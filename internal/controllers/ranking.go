...
func GetRanking(deps *config.Dependencies) httpio.HandlerWithError {
	return httpio.HandlerWithError(
		func(w http.ResponseWriter, r *http.Request) error {
			...
			orderDirection := "desc"
			paginatedResponse, err := models.Paginate[RankingItem](query, r, &httpio.PaginationParams{
				OrderBy:        orderBy,
			...
			if err := httpio.Encode(w, r, http.StatusOK, paginatedResponse); err != nil {
				return err
			}
...
func GetMyRanking(deps *config.Dependencies) httpio.HandlerWithError {
	return httpio.HandlerWithError(
		func(w http.ResponseWriter, r *http.Request) error {
