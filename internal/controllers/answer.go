...
func CreateAnswer(deps *config.Dependencies) httpio.HandlerWithError {
	return func(w http.ResponseWriter, r *http.Request) error {
		db := deps.DB
		answer, err := httpio.DecodeAndValidate[models.Answer](r, db)
		if err != nil {
			return err
		}

		// HTTP-level validation: current user must match answer.UserID
		currentUser := r.Context().Value(httpio.UserContextKey).(models.User)
		if answer.UserID != currentUser.ID {
			return httpio.InvalidUserError{}
		}
		...
		if existingAnswer.ID != 0 {
			return httpio.AppValidationError{
				...
				AppError:  httpio.AppError{Message: "already answered by current user"},
			}
		}
...
func UpdateAnswer(deps *config.Dependencies) httpio.HandlerWithError {
	return func(w http.ResponseWriter, r *http.Request) error {
		db := deps.DB
		answer, err := httpio.DecodeAndValidate[models.Answer](r, db)
		if err != nil {
		...
		if existingAnswer.PointsGrantedAt != nil {
			return httpio.AppValidationError{
				FieldPath: "question_id",
				AppError:  httpio.AppError{Message: "points already granted"},
			}
		}
